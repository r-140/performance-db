 ...
 * satellite data is stored on the leaf nodes only.
 * Internal nodes only hold keys and act as routers to the correct leaf node
 */
public class BPlusTreeIndex implements Index {
    private static final int ORDER = 4; // Порядок B+ дерева
    private String fileName;
//    private static final String FILE_NAME = "bplustree.dat"; // Имя файла для сохранения данных

    private Node root;

    // Конструктор B+ дерева
    public BPlusTreeIndex(String fileName) {
        this.fileName = fileName;
        root = new Node(true, fileName); // Создаем корневой листовой узел
    ...
    private static class Node implements Serializable {
        ...
        private List<Long> childrenOffsets; // Смещения дочерних узлов в файле
        private boolean isLeaf;
        private String fileName;

        // Конструктор узла
        Node(boolean isLeaf, String fileName) {
            keys = new ArrayList<>();
        ...
        private void split() {
            int mid = keys.size() / 2;
            Node left = new Node(isLeaf, fileName);
            ...
            Node right = new Node(isLeaf, fileName);
            right.keys.addAll(keys.subList(mid, keys.size()));
            right.values.addAll(values.subList(mid, values.size()));
            if (!isLeaf) {
                right.childrenOffsets.addAll(childrenOffsets.subList(mid, childrenOffsets.size()));
            }
            keys.clear();
            values.clear();
            childrenOffsets.clear();
            keys.add(right.keys.getFirst());
